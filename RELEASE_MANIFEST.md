# SlingToon Web 0.15.0 — Release Manifest

Data: 8 września 2026. Format: HTML5 Canvas / PWA, statyczny hosting GitHub Pages.
Pakiet kandydujący 0.15.0. Publikacja wymaga osobnej zgody właściciela po obejrzeniu wersji roboczej.
...
Face Studio ma dwa tryby głowy: domyślny wycinek zdjęcia bez przerysowania oraz portret rysunkowy. Zachowano duży lokalny portret, lokalne modele MediaPipe z weryfikacją sum kontrolnych, prywatność zdjęć, CSP, PWA i crop-free viewport. Nowe moduły fizyki, zapisu i renderowania interakcji są ujęte w cache aplikacji.

## Sprawdzenie

`npm run check`, `npm run build`, `npm run smoke`.

Szczegółowe wyniki i granice weryfikacji: [QA 0.15](docs/QA_0_15_PL.md).
...
Publikować po zgodzie właściciela do istniejącego repozytorium i GitHub Pages. Sprawdzić po wdrożeniu wersję `0.15.0`, mapę rozdziałów oraz aktualizację cache poprzedniej instalacji.

## Dalszy rozwój

Najpierw playtest nowej krzywej trudności odczutej ręką, jakości wycinka twarzy na prawdziwych zdjęciach oraz misji 38, 44, 58, 72, 74 i 78, które mają najmniejsze zmierzone okno tolerancji (±5–6). Płatności, reklamy, chmura i multiplayer pozostają poza zakresem.
