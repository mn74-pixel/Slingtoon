...
import { CAMPAIGN_ROUTES } from "./campaign-routes.js?v=0.14.0";
...
function mission(number, name, kind, x, y, mechanic, clue, win, interactions, options = {}) {
  ...
  const required = options.required ?? interactions.filter((item) => !["solid", "gate"].includes(item.type)).map((item) => item.id);
  ...
  return {
    ...
    goal: { kind, shape: "circle", x, y, radius: options.radius ?? (local === 0 || local === 4 ? 106 : number < 33 ? 99 : 92), scale: 1, motion: options.motion },
    ...
    visual: { accent: ["#ffd35f", "#5ce1bd", "#ff6078", "#a28bff"][Math.floor((number - 1) / 8) % 4], gag: options.gag ?? win, gagX: options.gagX, gagY: options.gagY, wash: "rgba(92,225,189,.015)", variant: local, companion: options.companion ?? chapter.id },
...
export const EXTRA_LEVELS = [
  ...
  mission(13, "Lody zanim się rozmyślą", "ice-cream", 930, 385, "DESER NA SPOKOJNIE", "Łagodny łuk wystarczy. To przerwa na lody, nie egzamin z celowania.", "Lody wybrały podróż w towarzystwie.", [], { motion: move("x", 32, .8), radius: 112 }),
  ...
  mission(21, "Przerwa na plankton", "sandwich", 965, 435, "ŁAGODNY DRYF", "Pod wodą potrzeba bardziej płaskiego strzału. Deser cierpliwie czeka.", "Kanapka zażyczyła sobie wyjścia na ląd.", [], { motion: move("y", 25, .7), radius: 110 }),
  ...
  mission(29, "Basen w basenie", "duck", 920, 450, "CHWILA NA DRYF", "Dziś nie ma przeszkód. Płyń łagodnym łukiem do hotelowego ratownika.", "Kaczka odmówiła pracy w mokrych warunkach.", [], { motion: move("x", 42, .75), radius: 108 }),
  ...
  mission(35, "Dźwig na kawę", "coffee", 1050, 290, "PODMUCH NAD ŁADUNKIEM", "Strumień z portowego wentylatora unosi ponad kontenerem.", "Operator dźwigu też zamówił cappuccino.", [wind("crane", 425, 130, 325, 430, 180, -1350, "W GÓRĘ ↑"), wall("cargo", 800, 450, 105)]),
  ...
  mission(37, "Kapitan szuka statku", "duck", 920, 420, "KOŁYSANIE NA SPOKOJNIE", "Kapitan łagodnie się kołysze. Spokojny strzał wystarczy.", "Statek był w drugiej kieszeni.", [], { motion: move("y", 32, .8), radius: 108 }),
  mission(38, "Pokład jest pod kątem", "bell", 1065, 435, "ODBIJ I ZADZWOŃ", "Opadnij na ukośny ponton. Po odbiciu traf w dzwonek przed bramką.", "Zaokrętowano pasażera bez użycia schodów.", [{ ...ramp("pontoon", 405, 435, 780, 560), thickness: 21 }, { ...button("gong", 880, 405), radius: 72 }, gate("gangway", 990, "gong")], { radius: 116 }),
  ...
  mission(41, "Balon na wolności", "balloon", 975, 355, "ŁAGODNIE RUCHOMY CEL", "Balon ma krótki, spokojny cykl. Przechwyć go wyższym łukiem.", "Balon twierdzi, że to Ty się zerwałeś.", [], { motion: move("y", 45, .9) }),
  ...
  mission(43, "Krzywe lustro skraca drogę", "ticket", 1070, 385, "PORTAL Z ZAKRĘTEM", "Lustro obraca kierunek lotu trochę w górę. Zachowuje jego prędkość.", "Lustro odmówiło prostowania bohatera.", [portal("mirror", 460, 390, 820, 330, -.25), wall("booth", 650, 185, 95)]),
  ...
  mission(45, "Przerwa techniczna na deser", "ice-cream", 955, 440, "DESER BEZ KOLEJKI", "Tylko Ty i deser. Zostaw trochę miejsca na następne atrakcje.", "Kontrola jakości zażądała drugiej porcji.", [], { radius: 110 }),
  ...
  mission(53, "Kawa przed odliczaniem", "coffee", 950, 440, "PRZERWA PRZED STARTEM", "Łagodny strzał. Kawa ma jeszcze ziemską grawitację.", "Odliczanie wstrzymano do pierwszego łyku.", [], { radius: 108 }),
  ...
  mission(58, "Ser przyciąga bardziej", "sandwich", 1050, 385, "POLE PRZYCIĄGANIA", "Okrąg pokazuje zasięg małego księżyca. Przeleć obok jego twardego środka.", "Księżyc zaprzecza, że jest z sera.", [planet("cheese", 600, 260, 225, 1000)]),
  mission(59, "Poczta księżycowa", "suitcase", 1060, 420, "LEKKI LOT, CIĘŻKA PACZKA", "W słabszej grawitacji leć płasko przez paczkę i nad niskim kraterem.", "Przesyłka dotarła przed listonoszem.", [crate("moon-post", 460, 205, 381, "NIE WSTRZĄSAĆ"), wall("crater", 790, 480, 100)]),
  ...
  mission(61, "Piknik bez okruszków", "sandwich", 950, 410, "ORBITALNA PRZERWA", "Kanapka odpływa powoli. Dostosuj łagodny strzał do słabszej grawitacji.", "Okruszki założyły własny układ planetarny.", [], { motion: move("y", 30, .7), radius: 108 }),
  ...
  mission(66, "Wentylacja zamiast korytarza", "coffee", 1070, 350, "PRĄD WENTYLACJI", "Oznaczony strumień przesuwa w prawo i delikatnie unosi. Omija niski przewód.", "Kawa otrzymała status obiektu latającego.", [current("ventilation", 380, 210, 435, 340, 360, -120, "WENTYLACJA →"), wall("duct", 825, 475, 90)]),
  ...
  mission(69, "Nie budzić kosmity", "alien", 950, 425, "CICHA WIZYTA", "Sąsiad dryfuje powoli. Delikatny strzał wystarczy, by złożyć wizytę.", "Kosmita mówi przez sen po ziemsku: jeszcze pięć minut.", [], { motion: move("x", 35, .7), radius: 108 }),
  ...
  mission(74, "Pamiątki bez cła", "suitcase", 1080, 385, "PACZKA, WIATR, PAMIĄTKI", "Przebij karton pamiątek. Szeroki strumień za nim pomaga utrzymać lot.", "Pamiątka z Księżyca chce wrócić na Księżyc.", [crate("souvenirs", 370, 180, 406, "PAMIĄTKI"), wind("tailwind", 570, 190, 330, 370, 380, -380, "Z WIATREM →")]),
  mission(75, "Objazd małej planety", "flag", 1080, 380, "DWA ŁAGODNE POLA", "Przeleć dolną stroną dwóch pól. Małe planety delikatnie podciągną tor.", "Planety pokłóciły się o pierwszeństwo.", [planet("moon-a", 450, 235, 205, 640), planet("moon-b", 820, 215, 215, 670)], { radius: 108 }),
  mission(76, "Powrót przez boczną galaktykę", "alien", 1080, 405, "DWIE RURY I DZWONEK", "Dwa skróty prowadzą do przycisku. Sąsiad pomylił galaktyki w adresie.", "Nawigacja mówi: zawróć przy najbliższym wszechświecie.", [portal("route-a", 380, 410, 610, 365), portal("route-b", 770, 365, 890, 300), button("arrival", 1000, 355)]),
  mission(77, "Ostatnia kawa w kosmosie", "coffee", 950, 435, "PRZERWA PRZED POWROTEM", "Spokojny lot do ostatniej kosmicznej kawy. Ziemia może chwilę poczekać.", "Kawa dopiła pasażera. Role się odwróciły.", [], { radius: 110, motion: move("x", 30, .65) }),
  mission(78, "Ziemia prosi najpierw zadzwonić", "bell", 1080, 410, "PRZYCISK, PACZKA, POWRÓT", "Naciśnij dzwonek Ziemi, przebij opakowanie pamiątki i przeleć przez otwartą bramkę.", "Ziemia mówi: wytrzyj buty z pyłu księżycowego.", [button("earth-bell", 405, 390), crate("gift", 680, 190, 396, "DLA DOMOWNIKÓW"), gate("earth-door", 925, "earth-bell")]),
  mission(79, "Lądowanie w pralni", "sock", 1070, 425, "PORTAL Z POWROTEM", "Pralnia przywraca ziemską grawitację. Wejdź do rury, potem otwórz drzwiczki.", "Po całej podróży nadal brakuje jednej skarpetki.", [portal("home-washer", 440, 390, 770, 295), button("laundry-button", 895, 355), gate("washer-door", 985, "laundry-button")], { scene: "laundry", environment: { gravity: 1, drag: 0 }, radius: 101 }),
  mission(80, "Jeszcze pięć minut. Tym razem serio.", "alarm", 1040, 440, "OSTATNI DZWONEK", "Zadzwoń do sypialni, potem wskocz do domowego skrótu. Budzik czeka od pierwszej misji.", "Budzik: gdzie byłeś przez te pięć minut?!", [button("home-bell", 405, 355), portal("pillow-tunnel", 615, 370, 835, 285), gate("bedroom-door", 950, "home-bell")], { scene: "bedroom", background: "assets/stage_morning_mayhem.svg", environment: { gravity: 1, drag: 0 }, radius: 106, tag: "DOBRANOC, WSZECHŚWIECIE!", gag: "DRZEMKA: 80 MISJI PÓŹNIEJ", gagX: 650, gagY: 90 }),
];
