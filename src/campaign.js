...
import { CAMPAIGN_ROUTES } from "./campaign-routes.js?v=0.39.0";
import { PROP_CLEARANCE, clearanceBetween, goalParts, propParts, spansVertically } from "./prop-art.js?v=0.39.0";
...
export function mission(number, name, kind, authoredX, authoredY, mechanic, clue, win, authoredInteractions, options = {}) {
  ...
  return {
    ...
    // One Move — sliding one piece of furniture before the shot — existed in a
    // single mission, so the switch beside it had nothing to switch to in the
    // other 87. Measured across the full -90..+100 slide: these keep every
    // spacing rule, keep the star readable, and stay winnable at every offset.
...
export const EXTRA_LEVELS = [
  ...
  mission(10, "Parawan z certyfikatem", "umbrella", 1050, 445, "PRZEBIJ I PRZELEĆ", "Papierowy parawan pęka. Kamień za nim trzeba ominąć górą.", "Zdobyto trzy centymetry plaży.", [crate("screen", 440, 185, 401, "TEREN PRYWATNY"), wall("rock", 750, 452, 95)], { editable: { id: "rock", minOffset: -90, maxOffset: 100 } }),
  ...
  mission(18, "Prąd nie wymaga ładowarki", "shell", 1080, 380, "PODWODNY PRĄD", "Turkusowy pas płynie w prawo. Przeleć nim pod rafą.", "Rachunek za prąd płaci węgorz.", [current("stream", 380, 255, 425, 260, 650, -80), wall("coral", 780, 100, 85, 165, "RAFA")], { editable: { id: "coral", minOffset: -90, maxOffset: 100 } }),
  ...
  mission(71, "Kanapka na orbicie", "sandwich", 1080, 340, "PLOMBA I GRAWITACJA", "Przebij folię lunchboxu. Pole małego księżyca lekko podciąga tor do kanapki.", "Ser obrał orbitę niezależną od pieczywa.", [crate("lunchbox", 365, 195, 391, "NIE OTWIERAĆ W PRÓŻNI"), planet("crumb-moon", 735, 210, 250, 900)], { editable: { id: "lunchbox", minOffset: -90, maxOffset: 100 } }),
  ...
  mission(74, "Pamiątki bez cła", "suitcase", 1080, 385, "PACZKA, WIATR, PAMIĄTKI", "Przebij karton pamiątek. Szeroki strumień za nim pomaga utrzymać lot.", "Pamiątka z Księżyca chce wrócić na Księżyc.", [crate("souvenirs", 370, 180, 406, "PAMIĄTKI"), wind("tailwind", 570, 190, 330, 370, 380, -380, "Z WIATREM →"), hazard("customs-beam", 900, 440, 70, 146, "KONTROLA!", "Skaner celny przechwycił pamiątki. Wyżej i dalej.")], { editable: { id: "souvenirs", minOffset: -90, maxOffset: 100 } }),
  ...
  mission(83, "Salami kontra trampolina", "sock", 1000, 440, "WAHADŁO I SPRĘŻYNA", "Odbij się mocno, a potem przeczekaj wahadło. Kolejność ma znaczenie, bo sprężyna nie czeka.", "Skarpetka widziała wszystko i nic nie powie.", [spring("launch", 400, 540, 560, 540), pendulum("ham", 730, 130, 230, .75, 1.1)], { scene: "laundry", editable: { id: "launch", minOffset: -90, maxOffset: 100 } }),
  ...
  mission(85, "Przerwa na kanapkę", "sandwich", 700, 300, "SPOKOJNY ODBIÓR", "Jedna sprężyna, żadnego pośpiechu. Sprawdź, jak mocne wejście daje jak wysoki wyskok.", "Kanapka skorzystała z przerwy wcześniej niż Ty.", [spring("break-pad", 400, 530, 580, 530, .6)], { scene: "garden", tag: "SMACZNEGO!", editable: { id: "break-pad", minOffset: -90, maxOffset: 100 } }),
