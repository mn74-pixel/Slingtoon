...
`scripts/balance-campaign.mjs` mierzył tolerancję ze stałym krokiem 4 px, więc przy oknie ±5 badał przesunięcia −5, −1 i +3, nigdy nie sprawdzając narożników pola. Raportowana tolerancja mogła być więc taka, której trasa nie przetrwała. Pomiar chodzi teraz po siatce lądującej dokładnie na ±margin, a test regresji porównuje każdą z 80 tras z zapisaną w `docs/campaign-balance.json` wartością. Skrypt wyklucza też gwiazdki wewnątrz stref zakazanych i w pasie ruchu ruchomych przeszkód.

Raport pomiaru jest generowany od nowa przy każdym wydaniu, dlatego `docs/campaign-balance.json` jest jednym żywym plikiem bez numeru wersji w nazwie.
